...
public class Vol {
	...
	public static int getCount(java.sql.Connection connection) {
		int count = 0;
		boolean nullConn = connection == null;
		if (nullConn)
			connection = database.Connect.getConnection();
		try {
			java.sql.PreparedStatement statement = connection.prepareStatement("SELECT COUNT(*) AS total FROM Vol");
			java.sql.ResultSet result = statement.executeQuery();
			if (result.next()) {
